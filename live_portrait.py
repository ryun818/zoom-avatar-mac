...
def recognize_from_video(models):
    ...
    try:
        ...
        pyvirtualcam = None
        vcam_started = False
        logger.info("pyvirtualcam not installed, virtual camera disabled")

    # face analysis for realtime tracking
    face_analysis_fn = models["face_analysis"]

    # source face landmarks for guide
    lmk_src = crop_info["lmk_crop"]
    lmk_all = lmk_src.reshape(-1, 2)
    h_s, w_s = src_img.shape[:2]

    # marker drawing helper
    def draw_markers(img, le, re, nose, mouth, color, thickness=2):
        cv2.circle(img, (int(le[0]), int(le[1])), 8, color, thickness)
        cv2.circle(img, (int(re[0]), int(re[1])), 8, color, thickness)
        cv2.line(img, (int(le[0])-12, int(le[1])), (int(le[0])+12, int(le[1])), color, 1)
        cv2.line(img, (int(re[0])-12, int(re[1])), (int(re[0])+12, int(re[1])), color, 1)
        cv2.circle(img, (int(nose[0]), int(nose[1])), 5, color, thickness)
        cv2.line(img, (int(mouth[0])-15, int(mouth[1])), (int(mouth[0])+15, int(mouth[1])), color, 1)

    # webcam crop helper
    def crop_cam_to_ratio(frame, target_w, target_h):
        h_c, w_c = frame.shape[:2]
        target_ratio = target_w / target_h
        cam_ratio = w_c / h_c
        if cam_ratio > target_ratio:
            crop_w = int(h_c * target_ratio)
            x_start = (w_c - crop_w) // 2
            return frame[:, x_start:x_start+crop_w], x_start, 0, crop_w, h_c
        else:
            crop_h = int(w_c / target_ratio)
            y_start = (h_c - crop_h) // 2
            return frame[y_start:y_start+crop_h, :], 0, y_start, w_c, crop_h

    # map detection coords to cropped preview coords
    def map_to_preview(pt, frame_shape, preview_w, preview_h, x_off, y_off, crop_w, crop_h):
        return ((pt[0] - x_off) / crop_w * preview_w,
                (pt[1] - y_off) / crop_h * preview_h)

    # draw realtime tracking markers on preview
    def draw_tracking(cam_resized, frame, preview_w, preview_h, x_off, y_off, crop_w, crop_h, color=(0, 255, 255)):
        trk_faces = face_analysis_fn(frame[:, :, ::-1])
        if len(trk_faces) > 0:
            trk_lmk = trk_faces[0]["landmark_2d_106"].reshape(-1, 2)
            tle = trk_lmk[33:43].mean(axis=0)
            tre = trk_lmk[87:97].mean(axis=0)
            tn = trk_lmk[72]
            tm = trk_lmk[52:72].mean(axis=0)
            dle = map_to_preview(tle, frame.shape, preview_w, preview_h, x_off, y_off, crop_w, crop_h)
            dre = map_to_preview(tre, frame.shape, preview_w, preview_h, x_off, y_off, crop_w, crop_h)
            dn = map_to_preview(tn, frame.shape, preview_w, preview_h, x_off, y_off, crop_w, crop_h)
            dm = map_to_preview(tm, frame.shape, preview_w, preview_h, x_off, y_off, crop_w, crop_h)
            draw_markers(cam_resized, dle, dre, dn, dm, color, 2)

    # --- Guide preview: source image + webcam ---
    logger.info("Press SPACE to start when your face is in the guide")
    src_bgr = src_img[:, :, ::-1]
    src_guide = src_bgr.copy()
    # draw source landmarks (red)
    src_left_eye = lmk_all[33:43].mean(axis=0)
    src_right_eye = lmk_all[87:97].mean(axis=0)
    src_nose = lmk_all[47]
    src_mouth = lmk_all[52:72].mean(axis=0)
    draw_markers(src_guide, src_left_eye, src_right_eye, src_nose, src_mouth, (0, 0, 255))

    while True:
        ret, frame = capture.read()
        if not ret:
            break
        cam_cropped, x_off, y_off, crop_w, crop_h = crop_cam_to_ratio(frame, w_s, h_s)
        cam_resized = cv2.resize(cam_cropped, (w_s, h_s))
        draw_tracking(cam_resized, frame, w_s, h_s, x_off, y_off, crop_w, crop_h)
        cv2.putText(cam_resized, "SPACE to start / Q to quit", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        preview = np.concatenate([cam_resized, src_guide], axis=1)
        ...
        if key == ord("q"):
            ...
            return
    logger.info("Starting LivePortrait...")
    ...
    while True:
        ...
        if pyvirtualcam is not None:
            if not vcam_started:
                vcam = pyvirtualcam.Camera(width=1280, height=720, fps=20, fmt=pyvirtualcam.PixelFormat.BGR)
                vcam_started = True
                logger.info(f"Virtual camera started: {vcam.device} (1280x720)")
            if vcam is not None:
                h_img, w_img = driving_img.shape[:2]
                ...
                y_off_v = (720 - new_h) // 2
                x_off_v = (1280 - new_w) // 2
                canvas[y_off_v:y_off_v+new_h, x_off_v:x_off_v+new_w] = scaled
                vcam.send(canvas)
                vcam.sleep_until_next_frame()

        # show webcam + avatar side by side
        if not args.cui or args.video:
            h_a, w_a = driving_img.shape[:2]
            cam_cropped, x_off, y_off, crop_w, crop_h = crop_cam_to_ratio(frame, w_a, h_a)
            cam_resized = cv2.resize(cam_cropped, (w_a, h_a))
            draw_tracking(cam_resized, frame, w_a, h_a, x_off, y_off, crop_w, crop_h)
            preview = np.concatenate([cam_resized, driving_img], axis=1)
            cv2.imshow("frame", preview)
            frame_shown = True
